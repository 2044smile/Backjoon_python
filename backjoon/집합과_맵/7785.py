...
(2 ≤ n ≤ 106) 다음 n개의 줄에는 출입 기록이 순서대로 주어지며, 각 사람의 이름이 주어지고
...
for _ in range(n):
    ...
    # 출입을 했으면 딕셔너리로 받는다.
    if state == "enter":
        temp[name] = state  # {Askar: enter, Artem: enter}
    # 퇴근을 했으면 삭제해준다.
    else:
        del temp[name]  # del Baha

# 사전 순의 역순으로 정렬한다. {Askar: enter, Artem: enter}
